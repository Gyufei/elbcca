...
export function useWorkflowParams(params: Record<string, any>, isVa: boolean) {
  ...
  function getSendUrl() {
    if (isVa) {
      if (isTransferOp) return userPathMap.vaSendTransfer;
      if (isSwapOp) return userPathMap.vaSendSwap;
      return null;
    }
